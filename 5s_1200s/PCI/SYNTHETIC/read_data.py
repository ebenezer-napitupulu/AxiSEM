...
obs = obspy.read("PCI_full.MSEED")       #change file name of file.MSEED

#####################################figure 1######################################
plt.figure(1)                        
fig = plt.figure(1)
fig.suptitle('PCI', fontsize=14)    #change the title of the each figure
...
vel_obs.legend()
#--------------------------------save figure-------------------------------
#plt.savefig('sintetik_PCI', format='png', dpi=1200)
plt.savefig('PCI sintetik and obervasi', format='png', dpi=1200)

###################################figure 2########################################
plt.figure(2)                       
fig2 = plt.figure(2)
fig2.suptitle("PCI", fontsize=14)
...
vel_obs.legend()
#------------------------------------save figure---------------------------
plt.savefig('PCI Filtered 0.3Hz-1000s', format='png', dpi=1200)

#####################################figure 3######################################
plt.figure(3)
fig3 = plt.figure(3)
fig3.suptitle("PCI", fontsize=14)
...
#------------------------------------save figure---------------------------
plt.savefig('PCI Filtered 0.3Hz-600s', format='png', dpi=1200)

#####################################figure 4######################################
plt.figure(4)
fig4 = plt.figure(4)
fig4.suptitle("PCI", fontsize=14)
...
tr_obs = obs[0]
obs_filt = tr_obs.copy()
obs_filt.filter('lowpass', freq=0.1, corners=2, zerophase=True)
t_obs = np.arange(0, obs[0].stats.npts/obs[0].stats.sampling_rate, obs[0].stats.delta)
...
vel_obs.set_xlim(0,600)
vel_obs.set_ylim(-9E+04,9E+04)
vel_obs.legend(bbox_to_anchor=(0.37,0.3), loc=1, borderaxespad=0.)
...
vel_.set_xlim(0,600)
vel_.set_ylim(-2E-04,2E-04)
vel_.legend()

#------------------------------save figure---------------------------------
plt.savefig('PCI Filtered 0.3Hz-600s Merged', format='png', dpi=1200)
plt.show()
